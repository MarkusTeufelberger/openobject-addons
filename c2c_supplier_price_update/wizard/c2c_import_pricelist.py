    ...
    def action_import(self, cr, uid, ids, context=None):
        ...
        for wiz in self.browse(cr, uid, ids, context):
            ...
            for i in range(len(reader_info)):
                ...
                prod_ids = prod_obj.search(cr, uid, [('ean13', '=', values['EAN13'])])
                
                if prod_ids and values['Supplier'] != '-':
                    for prod in prod_obj.browse(cr, uid, prod_ids):
                        prod_supplier_ids = prod_supplier_obj.search(cr, uid, [('product_id','=', prod.id), ('name.name', '=', values['Supplier'])])
                        if prod_supplier_ids: 
                            for prod_supplier in prod_supplier_obj.browse(cr, uid, prod_supplier_ids):
                                    prod_supplier_obj.write(cr, uid, prod_supplier.id, {'delay': values['Supplier Delay'],
                                                                                        ...
                                                                                        'product_code': values['Supplier Code'],
                                                                                        'product_name': values['Supplier Name']})

                                    if prod_supplier.pricelist_ids:
                                        price_ids = pricelist_obj.search(cr, uid, [('suppinfo_id','=', prod_supplier.id), ('min_quantity', '=', values['Quantity'])])
                                        if price_ids:
                                            pricelist_obj.write(cr, uid, price_ids, {'min_quantity': values['Quantity'],
                                                                                     'price': values['Price']})
                                        else:
                            ...
                            partner_ids = res_obj.name_search(cr, uid, values['Supplier'], args=None, operator='ilike', context=None, limit=80)
