...
#                    
#        days_delay=date_difference(cr,uid,old_planned_date,old_pick_date)
#       
#        move_ids=[]
...
#            next_move_dt=self.read(cr,uid,next_mv_id,['date_planned','picking_id','move_dest_id'])
#            new_move_date=(DateTime.strptime(next_move_dt['date_planned'], '%Y-%m-%d %H:%M:%S') + DateTime.RelativeDateTime(days=days_delay or 0)).strftime('%Y-%m-%d %H:%M:%S')
#            self.write(cr,uid,next_move_dt['id'],{'date_planned':new_move_date})
#        
#        res=[]
#        if all_pick_ids:
#            cr.execute("""select
