...
"""Inherits account_invoice to compute from button the early payment discount"""
...
class account_invoice(osv.osv):
    ...
    def _get_early_discount_amount(self, cr, uid, ids, field_name, arg, context):
        ...
        res = {}
        prod_early_payment_id = self.pool.get('product.product').search(cr, uid, [('default_code', '=', 'DPP')])[0]
    ...
    _columns = {
        ...
        'early_discount_amount': fields.function(_get_early_discount_amount, method=True, string="E.P. amount", type='float', digits=(16, int(config['price_accuracy'])), help="Early payment discount amount to apply.", readonly=True)
    }
    ...
    def compute_early_payment_lines(self, cr, uid, id):
        ...
        prod_early_payment_id = self.pool.get('product.product').search(cr, uid, [('default_code', '=', 'DPP')])[0]
        prod_early_payment = self.pool.get('product.product').browse(cr, uid, prod_early_payment_id)
        ...
        for early_payment_line in early_payments:
            ...
            for invoice_line in self.pool.get('account.invoice.line').browse(cr, uid, early_payments[early_payment_line]):
                ...
                elif prod_early_payment.property_stock_account_output and str(prod_early_payment.property_stock_account_output.id) in group_account_line[early_payment_line]:
    ...
    def onchange_partner_id(self, cr, uid, ids, type, part, date_invoice=False, payment_term=False, partner_bank=False):
        """extends this event for delete early payment discount if it isn't valid to new partner or add new early payment discount"""
        res = super(account_invoice, self).onchange_partner_id(cr, uid, ids, type, part, date_invoice=date_invoice, payment_term=payment_term, partner_bank=partner_bank)
        if not part:
            return res
    ...
    def onchange_payment_term(self, cr, uid, ids, payment_term, part=False):
        """onchanges event to update early payment dicount when the payment term changes"""
        res = {}
        if not payment_term:
            return res
